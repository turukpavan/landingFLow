...
const RecentProperties = ({ propertiesData, isLoading }) => {
  return (
    <section className="w-full bg-gradient-to-br from-[#653c6c] via-[#575ca6] to-[#25145A] text-white py-16 px-4 font-sans">
      <div className="max-w-[1300px] mx-auto">
        ...
        {/* Dynamic Property Grid Layer */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {isLoading ? (
            /* Active Shimmer Skeleton Loading State */
            [1, 2, 3].map((skeletonId) => (
              <div 
                key={skeletonId} 
                className="w-full flex flex-col text-left animate-pulse"
              >
                {/* Image Placeholder */}
                <div className="relative aspect-[16/11] w-full rounded-2xl bg-white/10 mb-5" />
                
                {/* Title Line Placeholder */}
                <div className="h-5 bg-white/20 rounded-md w-3/4 mb-3" />
                
                {/* Location Line Placeholder */}
                <div className="h-3 bg-white/10 rounded-md w-1/2 mb-4" />
                
                {/* Category Placeholder */}
                <div className="h-3 bg-white/10 rounded-md w-1/4 mb-4" />
                
                {/* Features Split Bar Placeholder */}
                <div className="flex justify-between items-center border-b border-white/10 pb-4 mb-4">
                  <div className="h-4 bg-white/20 rounded-md w-1/4" />
                  <div className="h-4 bg-white/10 rounded-md w-1/3" />
                </div>
                
                {/* Button Footer Row Placeholder */}
                <div className="flex justify-between items-center pt-1">
                  <div className="h-3 bg-white/5 rounded-md w-1/6" />
                  <div className="h-8 bg-white/20 rounded-lg w-28" />
                </div>
              </div>
            ))
          ) : !propertiesData || propertiesData.length === 0 ? (
            /* Empty Data Fallback State */
            <div className="col-span-full text-center py-12 text-purple-200/50 font-medium">
              No recent properties found available at the moment.
            </div>
          ) : (
            /* Content Active Render View */
