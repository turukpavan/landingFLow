...
const Header = () => {
  ...
  const location = useLocation(); // 2. Grab the current location object

  // 3. Removed the hardcoded 'active' properties
  const navItems = [
    { name: 'Home', href: '/' },
      ...
      <div className="max-w-[1300px] mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center bg-[#f6f6f6] border border-gray-100/50 rounded-full p-2">
          <ul className="flex items-center space-x-0.5">
            {navItems.map((item) => {
              // 4. Dynamically check if this item's path matches the browser path
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          ...
          <Link
            to="/login"
            className="group flex items-center space-x-1.5 sm:space-x-2 bg-[#feeae9] hover:bg-[#fee1d6] text-[#f12607] pl-3 sm:pl-4 pr-1 py-2 rounded-full text-xs font-semibold transition-all duration-200"
          >
            <span>Log out</span>
            <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-[#ed3a3a] group-hover:bg-[#d92828] flex items-center justify-center text-white transition-colors duration-200">
              <ButtonArrowIcon />
            </div>
          </Link>
      ...
      {isOpen && (
        <div className="lg:hidden bg-white border-b border-gray-100 px-4 py-3 shadow-lg">
          <ul className="space-y-1">
            {navItems.map((item) => {
              // 5. Apply the same dynamic check for the mobile menu dropdown
              const isActive = location.pathname === item.href;
